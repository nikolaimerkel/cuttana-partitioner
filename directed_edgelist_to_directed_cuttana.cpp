...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        ...
        cerr << "Converts edgelist to CUTTANA directed graph format" << endl;
        return 1;
    }

    string input_file = argv[1];
    string output_file = input_file + ".directed.cuttana";

    // Map to store adjacency list: vertex_id -> set of neighbors (outgoing edges)
    unordered_map<long long, unordered_set<long long>> adjacency_list;
    // Track all vertices (including those with only incoming edges)
    unordered_set<long long> all_vertices;
    ...
    while (ptr < end) {
        ...
        // Add only one direction (directed graph: src -> dst)
        ...
        if (edges_read % 1000000 == 0) {
            cout << "  Read " << edges_read / 1000000 << "M edges, "
                 << all_vertices.size() << " vertices so far..." << endl;
        }
    }
    ...
    cout << "  Finished reading: " << edge_count << " edges, "
         << all_vertices.size() << " vertices" << endl;

    // Collect and sort old vertex IDs
    cout << "Collecting vertex IDs..." << endl;
    vector<long long> sorted_old_ids;
    sorted_old_ids.reserve(all_vertices.size());
    for (long long old_id : all_vertices) {
    ...
    // Add self-loops for vertices with zero out-degree
    ...
    // Count total edges (directed: don't divide by 2)
    ...
    cout << "Directed edges: " << total_edges << endl;
    if (zero_degree_count > 0) {
        cout << "Added self-loops for " << zero_degree_count << " zero out-degree vertices" << endl;
    }
    ...
    // Write adjacency list for each vertex (outgoing edges)
